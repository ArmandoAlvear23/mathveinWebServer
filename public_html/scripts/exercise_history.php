...
if ($input){
    ...
    if (json_last_error() == JSON_ERROR_NONE){
        ...
        if (isset($json[0]["user_id"])){
            ...
            if($stmt=$mysqli->prepare("SELECT description, intensity, start_time, end_time, start_time_am_pm, end_time_am_pm, timestamp FROM exercises WHERE user_id = ? ORDER BY timestamp DESC LIMIT 20")){
				...
				$stmt->bind_result($db_description, $db_intensity, $db_start_time, $db_end_time, $db_start_time_am_pm, $db_end_time_am_pm, $db_timestamp);
				//Get user_id from database
				while($stmt->fetch()){
					$row_array['description'] = $db_description;
					$row_array['intensity'] = $db_intensity;
