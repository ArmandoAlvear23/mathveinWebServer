...
<html>
    <head
    <meta charset="UTF-8">
        <title> MATHVEIN-Exercise History</title>
        </head>
        <body>
            <center>
            =====================================</br>
			...
			<?php
            if($stmt=$mysqli->prepare("SELECT description, intensity, timestamp FROM exercises WHERE user_id=? ORDER BY timestamp DESC LIMIT 20")){
				$user_id = 1;
				$stmt->bind_param('i', $user_id);
                $stmt->execute();
                $stmt->bind_result($db_description, $db_intensity, $db_timestamp);
				echo("EXERCISE HISTORY</br>");
				echo "<table border=\"1\" align=\"center\">";
				echo "<tr><th>Type</th>";
				echo "<th>Intensity Dish</th>";
